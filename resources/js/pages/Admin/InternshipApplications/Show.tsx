...
export default function Show({ application }: Props) {
    ...
    const handleStatusUpdate = () => {
        patch(`/admin/internship-applications/${application.id}/status`);
    };

    const handleDelete = () => {
        if (confirm('Are you sure you want to delete this application?')) {
            router.delete(`/admin/internship-applications/${application.id}`);
        }
    };
    ...
    return (
        <AppLayout>
            <Head title={`Internship Application - ${application.name}`} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <Label className="text-gray-600">Field of Interest</Label>
                                    <p className="font-medium">{application.field_of_interest || 'Not specified'}</p>
                                </div>
                                <div>
                                    <Label className="text-gray-600">Duration</Label>
                                    <p className="font-medium">
                                        {application.duration_months ? `${application.duration_months} months` : 'Not specified'}
                                    </p>
                                </div>
                                {application.available_from && (
                                    <div className="col-span-2">
                                        <Label className="text-gray-600">Available From</Label>
                                        <p className="font-medium">{new Date(application.available_from).toLocaleDateString()}</p>
                                    </div>
                                )}
                            </div>
                        </div>

                        {application.why_internship && (
                            <div className="bg-white rounded-lg shadow p-6">
                                <h2 className="text-xl font-bold mb-4">Motivation</h2>
                                <Label className="text-gray-600">Why do you want to intern with us?</Label>
                                <p className="mt-2 text-gray-700 whitespace-pre-wrap">{application.why_internship}</p>
                            </div>
                        )}
                    </div>
